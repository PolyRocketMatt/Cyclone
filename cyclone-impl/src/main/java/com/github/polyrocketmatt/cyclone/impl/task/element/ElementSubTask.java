...
public class ElementSubTask extends ParallelTensorTask {

    private final TornadoNativeArray other;
    private final int size;
    private final String id;

    public ElementSubTask(@NotNull TornadoNativeArray buffer, @NotNull TornadoNativeArray other, int size) {
        super(buffer);
        this.other = other;
        this.size = size;
        this.id = "esub_%s".formatted(TaskUtils.randomIdentifier(8));
    }

    @Override
    public void resolve(@NotNull TaskGraph graph, @NotNull TensorType type) {
        switch (type) {
            case FLOAT      -> graph.task(id, ElementKernels::subFloat, (FloatArray) buffer, (FloatArray) other, size);
