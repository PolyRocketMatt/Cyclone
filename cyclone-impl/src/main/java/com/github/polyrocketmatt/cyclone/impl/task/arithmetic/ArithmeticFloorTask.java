...
public class ArithmeticFloorTask extends ParallelTensorTask {

    private float value;
    private final int size;
    private final String id;

    public ArithmeticFloorTask(@NotNull TornadoNativeArray buffer, int size) {
        super(buffer);
        this.size = size;
        this.id = "floor_%s".formatted(TaskUtils.randomIdentifier(8));
    }

    @Override
    public void resolve(@NotNull TaskGraph graph, @NotNull TensorType type) {
        switch (type) {
            case FLOAT      -> graph.task(id, ArithmeticKernels::floorFloat, (FloatArray) buffer, size);
