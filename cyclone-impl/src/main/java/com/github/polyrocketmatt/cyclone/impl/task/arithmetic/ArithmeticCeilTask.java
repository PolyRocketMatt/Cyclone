...
public class ArithmeticCeilTask extends ParallelTensorTask {

    private float value;
    private final int size;
    private final String id;

    public ArithmeticCeilTask(@NotNull TornadoNativeArray buffer, int size) {
        super(buffer);
        this.size = size;
        this.id = "ceil_%s".formatted(TaskUtils.randomIdentifier(8));
    }

    @Override
    public void resolve(@NotNull TaskGraph graph, @NotNull TensorType type) {
        switch (type) {
            case FLOAT      -> graph.task(id, ArithmeticKernels::ceilFloat, (FloatArray) buffer, size);
