...
public class SubtractionTask implements ArithmeticTask {
    ...
    public @NotNull CycloneBuffer<Float> execute(@NotNull CycloneBuffer<Float> buffer, @NotNull FloatArray output, @NotNull FloatArray values, int size) {
        ...
        TaskGraph graph = new TaskGraph("subtraction_graph")
                .transferToDevice(DataTransferMode.FIRST_EXECUTION, input, output, values, size)
                .task("subtraction", SubtractionTask::subFloat, input, output, values, size)
        ...
        try (TornadoExecutionPlan plan = new TornadoExecutionPlan(itg)) {
            ...
            throw new CycloneException("Failed to execute subtraction task!", ex);
        }

        return buffer;
    }

    private static void subFloat(FloatArray input, FloatArray output,
