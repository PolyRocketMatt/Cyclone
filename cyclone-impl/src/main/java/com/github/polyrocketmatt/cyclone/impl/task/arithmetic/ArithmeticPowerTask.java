...
public class ArithmeticPowerTask extends ParallelTensorTask {

    private float value;
    private final int size;
    private final String id;

    public ArithmeticPowerTask(@NotNull TornadoNativeArray buffer, float value, int size) {
        super(buffer);
        this.value = value;
        this.size = size;
        this.id = "pow_%s".formatted(TaskUtils.randomIdentifier(8));
    }

    @Override
    public void resolve(@NotNull TaskGraph graph, @NotNull TensorType type) {
        switch (type) {
            case FLOAT      -> graph.task(id, ArithmeticKernels::powFloat, (FloatArray) buffer, value, size);
