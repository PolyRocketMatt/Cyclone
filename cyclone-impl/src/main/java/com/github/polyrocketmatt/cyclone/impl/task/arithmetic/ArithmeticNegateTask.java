...
public class ArithmeticNegateTask extends ParallelTensorTask {

    private float value;
    private final int size;
    private final String id;

    public ArithmeticNegateTask(@NotNull TornadoNativeArray buffer, int size) {
        super(buffer);
        this.size = size;
        this.id = "neg_%s".formatted(TaskUtils.randomIdentifier(8));
    }

    @Override
    public void resolve(@NotNull TaskGraph graph, @NotNull TensorType type) {
        switch (type) {
            case FLOAT      -> graph.task(id, ArithmeticKernels::negateFloat, (FloatArray) buffer, size);
