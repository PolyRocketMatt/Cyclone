...
public class ArithmeticLogXTask extends ParallelTensorTask {

    private final float value;
    private final int size;
    private final String id;

    public ArithmeticLogXTask(@NotNull TornadoNativeArray buffer, float value, int size) {
        super(buffer);
        this.value = value;
        this.size = size;
        this.id = "logx_%s".formatted(TaskUtils.randomIdentifier(8));
    }

    @Override
    public void resolve(@NotNull TaskGraph graph, @NotNull TensorType type) {
        switch (type) {
            case FLOAT      -> graph.task(id, ArithmeticKernels::logXFloat, (FloatArray) buffer, value, size);
