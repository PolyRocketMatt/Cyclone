...
public class ArithmeticAtan2Task extends ParallelTensorTask {

    private final float value;
    private final int size;
    private final String id;

    public ArithmeticAtan2Task(@NotNull TornadoNativeArray buffer, float value, int size) {
        super(buffer);
        this.value = value;
        this.size = size;
        this.id = "atan2_%s".formatted(TaskUtils.randomIdentifier(8));
    }

    @Override
    public void resolve(@NotNull TaskGraph graph, @NotNull TensorType type) {
        switch (type) {
            case FLOAT      -> graph.task(id, ArithmeticKernels::atan2Float, (FloatArray) buffer, value, size);
