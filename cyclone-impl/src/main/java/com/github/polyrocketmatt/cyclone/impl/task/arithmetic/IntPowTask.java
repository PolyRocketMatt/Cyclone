...
public class IntPowTask implements ArithmeticTask {
    ...
    public IntPowTask(@NotNull TornadoNativeArray input, @NotNull TornadoNativeArray output,
                      int power, int size) {
        this.input = input;
        this.output = output;
        this.power = power;
    ...
    public void resolve(@NotNull TaskGraph graph, @NotNull CycloneBufferType type) {
        switch (type) {
            case FLOAT      -> graph.task(NameResolver.resolveName("add"),
                    IntPowTask::intPowFloat, (FloatArray) input, (FloatArray) output, power, size);
