...
public class MultiplicationTask implements ArithmeticTask {
    ...
    public MultiplicationTask(@NotNull TornadoNativeArray input, @NotNull TornadoNativeArray output,
    ...
    public void resolve(@NotNull TaskGraph graph, @NotNull CycloneBufferType type) {
        switch (type) {
            case FLOAT      -> graph.task("mul",
                    MultiplicationTask::mulFloat, (FloatArray) input, (FloatArray) output, (FloatArray) values, size);

            default         -> throw new UnsupportedOperationException("Unsupported buffer type: %s".formatted(type));
        }
    }

    public static void mulFloat(FloatArray input, FloatArray output,
